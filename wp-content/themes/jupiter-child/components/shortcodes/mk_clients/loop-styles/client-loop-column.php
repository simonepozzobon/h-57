...
<ul<?php echo $container_styles; ?>>
	...
	while ($view_params['query']->have_posts()):
        ...
        $tag = get_post_meta(get_the_ID(), '_url', true);
        $template_directory = site_url() . '/tag/' . $tag;
        $image_src = Mk_Image_Resize::resize_by_id( get_post_thumbnail_id(), 'full', false, false, $crop = false, $dummy = true);
	    ?>
	    <li>
			<div class="layout-client-container">
				<?php echo !empty($tag) ? '<a target="' . $view_params['target'] . '" href="' . $template_directory . '">' : ''; ?>
				<div title="<?php the_title_attribute(); ?>" class="client-logo sp-client" style="background-color:#fff; -webkit-mask-image:url(<?php echo $image_src; ?>); mask-image:url(<?php echo $image_src; ?>); -webkit-mask-size: contain; mask-size: contain; mask-repeat: no-repeat; -webkit-mask-repeat: no-repeat; mask-position: center; -webkit-mask-position: center; <?php echo $view_params['height']; ?>"></div>
				<?php echo !empty($tag) ? '</a>' : ''; ?>
				<!--<div class="layout-client-name">
					<?php echo !empty($tag) ? '<a target="' . $view_params['target'] . '" href="' . $template_directory . '">' : ''; ?>
					<h3 class="master-holder"><span><?php the_title_attribute(); ?></span></h3>
					<?php echo !empty($tag) ? '</a>' : ''; ?>
				</div>-->
			</div>
	    </li>
