...
class Mk_Static_Files
{
    ...
    function __construct($with_actions = true) {
        ...
        if ($with_actions) {
            ...
            if (get_option("mk_theme_options_css_file") == "") $priority = 999;
    ...
    static function StoreAsset($folder, $filename, $file_content) {
        ...
        $mkfs = new Mk_Fs( array( 'context' => $folder ) ) ;
    ...
    static function StoreGlobalAssets($merged_assets = array(), $store_options = array() , $minify = true) {
        ...
        $mkfs = new Mk_Fs( array( 'context' => $upload_dir ) ) ;
        ...
        foreach ($store_options as $extension => $filename) {
            ...
            if (get_option($filename . "_sha1") != $sha1_concat_string or !$mkfs->exists($file_path)) {
                    $mkfs->put_contents($upload_dir . '' . $filename, $concat_string);
                    update_option($filename . "_sha1", $sha1_concat_string);
            }
        }
    }
    ...
    static function StoreThemeOptionStyles($minify = true) {
        ...
        if (self::deleteFileNew($folder, $filename) != true) {
        ...
        update_option("mk_theme_options_css_file", $filename, true);
    }
    ...
    public function DeleteThemeOptionStyles($remove_cache_plugins = false) {
        ...
        if ($this->deleteFileNew($folder , $filename) != true and $filename != "") {
            die("A problem occurred while trying to delete theme-options css file");
        } 
        else {
            update_option("mk_theme_options_css_file", "", true);
            return true;
        }
    }
        ...
        if (is_array($merged_assets) && !empty($merged_assets)) {
            ...
            foreach ($merged_assets as $asset) {
                ...
                if(!$wp_file_body or $mk_dev) {
                    ...
                    if ($mkfs->exists($dest_path . $filename)) {

                       $wp_get_file_body = $mkfs->get_contents($dest_path.$filename);
    ...
    static function addThemeOptionsCSSToEnqueue() {
        ...
        $theme_options = get_option( "mk_theme_options_css_file" );
    ...
    function critical_path_css() {
        ...
        if (!$critical_path_css_body) {
            ...
            $critical_path_css_body = $mkfs->get_contents($dest_path . $critical_path_css_filename);
    ...
    static function delete_global_assets($clear_db = true) {
        $dest_path = self::get_global_asset_upload_folder("directory");
        $mkfs = new Mk_Fs( array( 'context' => $dest_path ) ) ;
    ...
    static function deleteFile($filename) {
        ...
        $mkfs = new Mk_Fs( array( 'context' => $filename ) ) ;
        if (!$mkfs->exists($file)) return true;
        return $mkfs->delete($file);
    }


    /**
     * If the file does not exist or is deleted, return true.
     * When something goes wrong returns false.
     * @param $folder
     * @param $filename
     * @return bool
     ...
     * @since       Version 5.8
     */
    static function deleteFileNew($folder, $filename) {
        $mkfs = new Mk_Fs( array( 'context' => $folder ) ) ;
    ...
    function minify_html($input) {
        ...
        // Strip extra spaces and redundant comment lines in html
        $input = preg_replace(
            array(
                // t = text
                // o = tag open
